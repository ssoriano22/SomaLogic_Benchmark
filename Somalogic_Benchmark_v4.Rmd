...
annotate_figure(fig_eLoDFilt_AptCount, top = text_grob("Aptamer Count per Sample", size = 14))
ggsave("fig_eLoDFilt_AptCount.png")
```
...
sample_int_dist_mouse_plot = sample_var_mouse_df %>%
  ggplot(aes(x = Sample_ID, y = Intensity, fill = Group_final)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Intensity Distribution per Sample: Mouse",
       subtitle = "Filter: eLoD = M_blank + 4.9MAD_blank") +
  theme_bw() +
...
sample_int_dist_mouse_alt_plot = sample_var_mouse_alt_df %>%
  ggplot(aes(x = Sample_ID, y = Intensity, fill = Group_final)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Intensity Distribution per Sample: Mouse (Pre-Normalization)",
       subtitle = "Filter: eLoD = M_blank + 4.9MAD_blank") +
  theme_bw() +
  #stat_mean_sd_text(size = 2, y.pos = 6000) +
  ylim(0,6000) +
...
sample_int_dist_human_plot = sample_var_human_df %>%
  ggplot(aes(x = reorder(Sample_ID,row_ID), y = Intensity, fill = Clinical.Diagnosis)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Intensity Distribution per Sample: Human",
       subtitle = "Filter: eLoD = M_blank + 4.9MAD_blank",
       x = "Samples") +
  theme_bw() +
  #stat_mean_sd_text(size = 2, y.pos = 6000) +
...
sample_int_dist_human_alt_plot = sample_var_human_alt_df %>%
  ggplot(aes(x = reorder(Sample_ID,row_ID), y = Intensity, fill = Clinical.Diagnosis)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Intensity Distribution per Sample: Human (Pre-Normalization)",
       subtitle = "Filter: eLoD = M_blank + 4.9MAD_blank",
       x = "Samples") +
  theme_bw() +
  #stat_mean_sd_text(size = 2, y.pos = 6000) +
...
aptamer_CVgroup_mouse_plot = aptamer_CVgroup_mouse_df %>%
  ...
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Aptamer CV Distribution (By Group): Mouse",
       subtitle = "Outliers are hidden",
       x = "") +
  theme_bw() +
  ylim(0, 100) +
  ...
  theme(axis.title = element_text(size = 14)) +
  theme(title = element_text(size = 14))
ggsave("aptamer_CVgroup_mouse_plot.png")
...
aptamer_CVgroup_human_plot = aptamer_CVgroup_human_df %>%
  ggplot(aes(x = reorder(Clinical.Diagnosis, row_ID), y = CV, fill = Clinical.Diagnosis)) +
  geom_boxplot(outlier.shape = NA) +
  labs(title = "Aptamer CV Distribution (By Group): Human",
       subtitle = "Outliers are hidden",
       x = "") +
  theme_bw() +
  ylim(0, 100) +
...
fig_WiT_Dilution_Mouse = ggarrange(WiT_A_mouse_volcano_plot, WiT_B_mouse_volcano_plot, WiT_C_mouse_volcano_plot, labels="AUTO", ncol = 3, nrow = 1)
annotate_figure(fig_WiT_Dilution_Mouse, top = text_grob("Wilcox Test for Proteins in SomaLogic Study: Mouse", size = 14))
ggsave("fig_WiT_Dilution_Mouse.png")
```
...
WiT_boxDS_mouse_plot = WiT_boxDS_mouse_df %>%
  ...
  labs(title = "Fold Change Distribution per Dilution Comparison: Mouse",
       subtitle = "eLoD Filter, Pre-Normalization",
       x = "",
       y = "Log2(FC)") +
  theme_bw() +
  #stat_mean_sd_text(size = 2, y.pos = 6000) +
  ...
  theme(legend.title = element_text(size = 12)) +
  theme(title = element_text(size = 14))
ggsave("WiT_boxDS_mouse_plot.png")
...
fig_WiT_Dilution_Human = ggarrange(WiT_A_human_volcano_plot, WiT_B_human_volcano_plot, WiT_C_human_volcano_plot, labels="AUTO", ncol = 3, nrow = 1)
annotate_figure(fig_WiT_Dilution_Human, top = text_grob("Wilcox Test for Proteins in SomaLogic Study: Human", size = 14))
ggsave("fig_WiT_Dilution_Human.png")
```
...
WiT_boxDS_human_plot = WiT_boxDS_human_df %>%
  ...
  geom_boxplot(alpha = 0.5) +
  #geom_text(aes(label = median(FC))) +
  labs(title = "Fold Change Distribution per Dilution Comparison: Human",
       subtitle = "eLoD Filter, Pre-Normalization",
       x = "",
       y = "Log2(FC)") +
  theme_bw() +
  ...
  theme(legend.title = element_text(size = 12)) +
  theme(title = element_text(size = 14))
ggsave("WiT_boxDS_human_plot.png")
...
NCBI_KEGG_anno_df = nomConvert_Human_Mouse(soma_KMC_df$SOMA_Target)

soma_KMC_df = merge(soma_KMC_df,NCBI_KEGG_anno_df, by.x = "SOMA_Target", by.y = "HOM_Gene", all.x = TRUE) %>% unique()
# Filter out human/soma genes with no mouse match
soma_KMC_filt_df = soma_KMC_df %>% filter(!is.na(MUS_Gene)) %>% mutate(MUS_Gene = tolower(MUS_Gene))
...
# Calculate seer --> soma match overlap
seer_KMC_filt_df = seer_KMC_df %>% filter(!is.na(Genes)) %>% dplyr::select(c(Genes)) %>% unique()
NCBI_KEGG_Seer_anno_df = nomConvert_Mouse_Human(seer_KMC_filt_df$Genes)
seer_KMC_filt_df = merge(seer_KMC_filt_df,NCBI_KEGG_Seer_anno_df, by.x = "Genes", by.y = "MUS_Gene", all.x = TRUE)
colnames(seer_KMC_filt_df)[colnames(seer_KMC_filt_df) == "Genes"] = "MUS_Gene"
seer_KMC_filt_df = seer_KMC_filt_df %>% filter(!is.na(HOM_Gene)) %>% mutate(HOM_Gene = toupper(HOM_Gene))
seer_sscombo_KMC_df = merge(seer_KMC_filt_df,soma_KMC_df, by.x = "HOM_Gene", by.y = "SOMA_Target")
seer_sscombo_KMC_protein_overlap = seer_sscombo_KMC_df %>% dplyr::select(c("MUS_Gene.x","HOM_Gene","MUS_GeneDescription.x","HOM_UniProt.x","MUS_UniProt.x")) %>%
                                                            mutate(MUS_Gene.x = tolower(MUS_Gene.x)) %>%
                                                            unique() #%>% nrow()
colnames(seer_sscombo_KMC_protein_overlap) = c("MUS_Gene","HOM_Gene","GeneDescription","HOM_UniProt","MUS_UniProt")
...
sscombo_KMC_df = merge(seer_KMC_df,soma_KMC_filt_df, by.x = "Genes", by.y = "MUS_Gene") #Intersecting UniProt IDs btwn Soma + Seer
colnames(sscombo_KMC_df)[colnames(sscombo_KMC_df) == "UniProt.x"] = "Seer_UniProt"
colnames(sscombo_KMC_df)[colnames(sscombo_KMC_df) == "UniProt.y"] = "SOMA_UniProt"

#Generate table of protein overlaps (removing NP replicates)
protID_overlap = sscombo_KMC_df %>% dplyr::select(c(Genes,SOMA_Target,SOMA_TargetFullName,SOMA_UniProt,Seer_UniProt)) %>% unique()
#protID_gene_overlap = sscombo_KMC_df %>% dplyr::select(c(Genes,SOMA_Target,SOMA_TargetFullName)) %>% unique()
colnames(protID_overlap) = c("MUS_Target","SOMA_HUM_Target","SOMA_TargetFullName","SOMA_UniProt","Seer_UniProt")
#Export table of overlap proteins
write_tsv(protID_overlap,"protID_overlap_SomatoSeer.tsv")

sscombo_KMC_sig_df = sscombo_KMC_df %>% filter(Significance_wt == "Y")
sigprotID_KMC_overlap = sscombo_KMC_sig_df %>% dplyr::select(c(Genes)) %>% unique() #%>% nrow()

#Check for difference btwn soma --> seer and seer --> soma overlaps
colnames(protID_overlap) = c("MUS_Gene","HOM_Gene","GeneDescription","HOM_UniProt","MUS_UniProt")
overlap1 = protID_overlap %>% dplyr::select(c("MUS_Gene","HOM_Gene"))
overlap2 = seer_sscombo_KMC_protein_overlap %>% dplyr::select(c("MUS_Gene","HOM_Gene"))
overlap_method_diff = dplyr::setdiff(overlap1,overlap2)
overlap_method_diff2 = merge(protID_overlap,seer_sscombo_KMC_protein_overlap, by = "MUS_Gene", all = TRUE)
num_overlap_prot_Soma_unique = overlap_method_diff2 %>% filter(is.na(HOM_Gene.y)) %>% nrow()
num_overlap_prot_Seer_unique = overlap_method_diff2 %>% filter(is.na(HOM_Gene.x)) %>% nrow()
summary(comparedf(overlap1,overlap2))
```
...
# soma_FC = sscombo_KMC_df$SOMA_Diff_Median
# seer_FC = sscombo_KMC_df$diff_median
sscombo_KMC_corr_df = sscombo_KMC_df %>% rowwise() %>% mutate(data_FC = list(c(SOMA_Diff_Median,diff_median))) %>%
...
sscombo_KMC_corr_overlap_df = sscombo_KMC_corr_df %>% dplyr::select(c("AptName","AptName_SOMATarget","Genes","Seer_UniProt","SOMA_Target","SOMA_TargetFullName","SOMA_UniProt","Quality")) %>% unique()
...
quality_overlap_plot = sscombo_KMC_corr_overlap_df %>%
  ...
  dplyr::group_by(Quality) %>%
  summarise(Apt_Count = n(), Percent = round((Apt_Count/527)*100,digits = 0), Perc_Label = paste(as.character(Percent),"%",sep = "")) %>% #Total number of rows in sscombo_KMC_corr_overlap_df, higher than 476 because of 2+ aptamers targeting single protein
  ggplot(aes(x = Quality, y = Apt_Count, fill = Quality)) +
  geom_bar(stat = "identity") +
  ...
  labs(title = "Aptamer Quality of Overlap Proteins: KMC-PDAC v. KMC Control",
       ...
       y = "Aptamer Count") +
  theme_bw() +
  scale_y_continuous(limits = c(0,550), breaks = c(100,200,300,400,500)) +
  geom_hline(yintercept = 527, color = "purple4") +
  geom_text(aes(2,527,label = "Total Aptamers in Overlap: 527", vjust = -1),color = "purple4") +
  theme(legend.position = "none")
ggsave("quality_overlap_plot.png")
quality_overlap_plot

#Generate table of protein overlaps (removing NP replicates)
# protID_overlap2 = sscombo_KMC_corr_df %>% dplyr::select(c(Gene_Merge,SOMA_Target,SOMA_TargetFullName,SOMA_UniProt,Genes,Seer_UniProt,Quality)) %>% unique()
# colnames(protID_overlap2) = c("Gene_Merge","SOMA_Target","SOMA_TargetFullName","SOMA_UniProt","Seer_Target","Seer_UniProt","Quality")

#FC correlation graph - filtered mouse
corr_sscombo_KMC_plot = sscombo_KMC_corr_df %>%
  #filter(Significance_wt == "Y") %>%
  ggplot() +
  ...
  labs(title = "SomaLogic vs. Seer Fold Change (FC)",
       subtitle = "KMC-PDAC v. KMC Control, All Overlap IDs",
...
#Find number of protein IDs (removing NP duplicates) in seer KMC data - AFTER all filters
